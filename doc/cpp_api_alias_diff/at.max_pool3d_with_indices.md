## [API 别名]at::max_pool3d_with_indices

### PyTorch C++ API
```cpp
at::max_pool3d_with_indices(self, kernel_size, stride={}, padding=0, dilation=1, ceil_mode=false)
```

### Paddle C++ API
```cpp
paddle::experimental::max_pool3d_with_index(x, kernel_size, strides={1, 1, 1}, paddings={0, 0, 0}, dilations={1, 1, 1}, global_pooling=false, adaptive=false, ceil_mode=false)
```

两者功能一致，但 API 名称不同，PyTorch 为 `max_pool3d_with_indices`，Paddle 为 `max_pool3d_with_index`。参数映射具体如下：
